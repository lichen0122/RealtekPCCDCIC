...
Exports:
    ...
    - GitSSHManager: Core manager class
        - GitSSHManager.is_portable_git_valid: Check whether a PortableGit directory is complete
        - GitSSHManager.download_portable_git: Download and extract PortableGit (pure Python, no Qt)
        - GitSSHManager.ensure_portable_git: Validate-or-download in one call
...
class GitSSHManager:
    ...
    # ------------------------------------------------------------------
    # PortableGit Auto-Download (static methods)
    # ------------------------------------------------------------------
    ...
    def ensure_portable_git(
        ...
        *,
        ...
        terminal progress bar is used so the caller can simply write::
            ...
            GitSSHManager.ensure_portable_git(verbose=True)
        ...
        """
        if GitSSHManager.is_portable_git_valid(extract_dir):
            return True

        # verbose mode: use built-in terminal callbacks when caller didn't provide their own
        if verbose:
            if on_progress is None:
                on_progress = GitSSHManager._print_progress_bar
            if on_phase is None:
                on_phase = print
        ...
        try:
            GitSSHManager.download_portable_git(
